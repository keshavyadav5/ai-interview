import { Briefcase, ChartLine, FileUp, Loader2, Mic, User, User2 } from 'lucide-react'
...
const interview = [
  {
    icon: <User2 className='text-green-600 text-xl' />,
  ...
  {
    icon: <ChartLine className='text-green-600 text-xl' />,
      ...
      <div className='w-full max-w-6xl bg-white rounded-3xl shadow-2xl grid md:grid-cols-2 overflow-hidden'>
        ...
        >
          ...
          <p className='text-gray-600 mb-10'>
            Practise real interview scenarios powered by AI.
          ...
          <div className='space-y-2'>
            {
              interview.map((item, index) => (
                <motion.div key={index}
                  ...
                  className='flex items-center space-x-4 shadow-sm cursor-pointer py-2 bg-white pl-1 rounded'
                >
                  {item.icon}
        ...
        >
          ...
          <div className='space-y-2'>
            ...
            <div className='relative'>
              <Briefcase className='absolute top-4 left-4 text-gray-400' />
            ...
            <select
              ...
              className='w-full py-3 px-4 border border-gray-200 rounded-xl focus:ring-2 focus-within:ring-green-500 outline-none transition'>
            ...
            {!analysisDone && (
              ...
              >
                <FileUp className='text-4xl mx-auto text-green-600 mb-3' />
                <input type="file"
                ...
                </p>
                {
                  resumeFile && (
                    <motion.button
                      ...
                      className='mt-4 bg-gray-900 text-white px-5 py-2 rounded-lg hover:bg-green-800 transition'
                    >
                      {analyzing ? (
                  ...
                  skills.length > 0 && (
                    <div>
                      ...
                      <div className='flex flex-wrap gap-2'>
                        {skills.map((p, i) => <span key={i} className='bg-green-50 text-green-700 px-3 py-1 rounded-full text-sm'> {p}</span>)}
                      </div>
                    </div>
                  )
                }
          ...
          <motion.button
            ...
            className={`w-full py-3 rounded-full text-lg font-semibold transition-all duration-300 shadow-md mt-4 flex items-center justify-center
             ${!role || !experience
                ? "bg-gray-600 cursor-not-allowed"
                : "bg-green-600 hover:bg-green-700"
              } text-white`}
          >
            {loading ? <div className='flex items-center gap-3'><span className='hidden md:flex'>Processing..</span><Loader2 className='w-5 h-5 text-center animate-spin'/></div>: "Start Interview"}
          </motion.button>
        </motion.div>
